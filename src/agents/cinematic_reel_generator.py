...
class CinematicReelGenerator:
    ...
    def _generate_cinematic_images(self, prompts: List[str], output_dir: Path) -> List[Path]:
        """Generate 9:16 cinematic images via Replicate (Ideogram v2)."""
        import replicate
        ...
        for i, prompt in enumerate(prompts, 1):
            logger.info("[Cinematic Image %d/%d] Generating...", i, len(prompts))
            ...
            try:
                ...
                if res.status_code == 200:
                    p = output_dir / f"image_{i:02d}.png"
                    with open(p, "wb") as f:
                        f.write(res.content)
    ...
    def _build_cinematic_clips(
        ...
        for i, (img, text) in enumerate(zip(image_paths, lines), 1):
            ...
            # Note: fonts/fontfile might need to be adjusted for the environment
            filter_complex = (
    ...
    def _mix_music(self, video_path: Path, output_path: Path, volume: float) -> None:
        ...
        cmd = [
            ...
            "-filter_complex",
            f"[1:a]aloop=loop=-1:size=100M,volume={volume}[bg];[0:a][bg]amix=inputs=2:duration=first[a]",
            "-map", "0:v", "-map", "[a]",
            "-c:v", "copy", "-c:a", "aac", "-b:a", "192k",
            str(output_path),
        ]
        # If video has no audio (cinematic clips don't by default), amix might fail
        # So we use a simpler filter if needed, or just map the background music
        cmd_no_v_audio = [
            "ffmpeg", "-y",
            "-i", str(video_path), "-i", str(music),
    ...
    # ------------------------------------------------------------------
    # Script Generation (The logic found in the broken file)
    # ------------------------------------------------------------------
